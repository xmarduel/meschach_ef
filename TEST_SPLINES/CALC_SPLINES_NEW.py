...
from math import *
#
# General Splines :  with bc y'(X0)  = y'(X4)  = 0
#                            y''(X0) = y''(x4) = 0
#
#                     and  y(X0)  = y(X4)  = 0
#                          y(X2)  = 4.0
#
# unknowns are y''(X1), y''(X2), y''(X3), y(X1), y(X3)
#
#
from Numeric import *
...
coeffs = zeros((4,4),Float) # 4 coeffs on the 4 intervals [X0-X1],[X1-X2],[X2-X3],[X3-X4] 
#
# the mesh
mesh = zeros(4,Float)
#
...
sol  = zeros((5,1),Float)
#
y_xx  = zeros(5,Float)   # y''(X0) = y''(x4) = 0
y     = zeros(5,Float)   # y(X0)   = y(X4)   = 0 ; y(X2)  = 4.0
#
# the points to interpolate
y[0] = 0.0
y[1] = 0.0 # infact unknown -> to be calculated . Initialize with 0.0
y[2] = 4.0
y[3] = 0.0 # infact unknown -> to be calculated . Initialize with 0.0
y[4] = 0.0
#
# the mesh values
mesh[0] = 1.0
...
mat[0][1] =    -(mesh[0]) 
#
mat[1][0] =  6.0/(mesh[0]) + 6.0/mesh[1] 
mat[1][1] =  2.0*(mesh[0]+mesh[1]) 
mat[1][2] =              (mesh[1]) 
#
mat[2][0] = -6.0/(mesh[1]) 
mat[2][1] =      (mesh[1]) 
mat[2][2] =  2.0*(mesh[1]+mesh[2]) 
mat[2][3] =              (mesh[2])
mat[2][4] = -6.0/(mesh[2]) 
#
mat[3][2] =     (mesh[2]) 
mat[3][3] = 2.0*(mesh[2]+mesh[3]) 
mat[3][4] = 6.0/(mesh[2]) + 6.0/(mesh[3])
#
