...
from math import *
#
# Natural Splines :  y_xx = 0 at X0 and X4 
#
from Numeric import *
...
h    = zeros(4,Float)
#
# for the resolution : eval y_xx[1:3] <- sol
mat  = zeros((3,3),Float)
vec  = zeros((3,1),Float)
sol  = zeros((3,1),Float)
#
...
#
# and the rhs
vec[0] = -6.0/h[0]*( y[1]-y[0] ) + 6.0/h[1]*( y[2]-y[1] )
vec[1] = -6.0/h[1]*( y[2]-y[1] ) + 6.0/h[2]*( y[3]-y[2] )
vec[2] = -6.0/h[2]*( y[3]-y[2] ) + 6.0/h[3]*( y[4]-y[3] )
#
#
...
for i in range(4):
    #
    print(" i = ",i)
    coeffs[i][0] = 1.0/(6.0* h[i]) * ( y_xx[i+1] - y_xx[i] )
    coeffs[i][1] = 0.5 * y_xx[i]
    coeffs[i][2] = 1.0/(h[i]) * ( y[i+1] - y[i] ) - (1.0/6.0)* h[i] * ( y_xx[i+1] + 2*y_xx[i] )
    coeffs[i][3] = y[i]
    #
#
#
