	...
	int	n)
{
	...
	float	tx, ty, left, right, bottom, top;
	float	dashlen, a, b, c, d;
	...
	if (plotdev.markerspacing && plotdev.marker != (char *)NULL) {
		if (plotdev.markerscale != 0.0) {
			...
			centertext(1);
			/*clipping(0);*/
			tx = plotdev.markerscale * TEXTWIDTH * (WhatX(plotdev.axes[XIND].max) - WhatX(plotdev.axes[XIND].min));
			ty = plotdev.markerscale * TEXTHEIGHT * (WhatY(plotdev.axes[YIND].max) - WhatY(plotdev.axes[YIND].min));
