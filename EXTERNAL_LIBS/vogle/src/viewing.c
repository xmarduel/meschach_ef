 ...
 */
static void
normallookat(float vx, float vy, float vz, float px, float py, float pz)
 ...
 */
static void
lookatwithup(float vx, float vy, float vz, float px, float py, float pz)
{
	Vector	t, u, s;
	Matrix	tmp;
	double	dy, dz, lt, lu;
	...
	if (lt == 0.0)
		verror(VERR_BADCOORD, "can't have eyepoint and reference point the same in lookat.\n");

	lu = sqrt(u[V_X] * u[V_X] + u[V_Y] * u[V_Y] + u[V_Z] * u[V_Z]);

	if (lu == 0.0)
		verror(VERR_BADVECTOR, "up");
	...
	if (fabs(dz) >= 1.0)
		verror(VERR_BADVECTOR, "up vector and direction of view are the same.\n");
 ...
 */
void
lookat(float vx, float vy, float vz, float px, float py, float pz, float twist)
{
	if (!vdevice.initialised) 
		verror(VERR_UNINIT, "lookat");
 ...
 */
void
perspective(float fov, float aspect, float hither, float yon)
{
	Matrix		mat;

	if (!vdevice.initialised)
		verror(VERR_UNINIT, "perspective");

	if (aspect == 0.0)
		verror(VERR_BADASPECT, "perspective");

	if ((yon - hither) == 0.0)
		verror(VERR_BADPLANE, "perspective: near clipping plane same as far one.");

	if (fov == 0.0 || fov == 180.0)
		verror(VERR_BADFOV, "perspective");
 ...
 */
void
window(float left, float right, float bottom, float top, float hither, float yon)
{
	Matrix		mat;

	if (!vdevice.initialised)
		verror(VERR_UNINIT, "window");

	if ((right - left) == 0.0)
		verror(VERR_BADPLANE, "window: left clipping plane same as right one.");

	if ((top - bottom) == 0.0)
		verror(VERR_BADPLANE, "window: bottom clipping plane same as top one.");

	if ((yon - hither) == 0.0)
		verror(VERR_BADPLANE, "window: near clipping plane same as far one.");
 ...
 */
void
ortho(float left, float right, float bottom, float top, float hither, float yon)
{
	Matrix		mat;

	if (!vdevice.initialised)
		verror(VERR_UNINIT, "ortho: vogle not initialised");

	if ((right - left) == 0.0)
		verror(VERR_BADPLANE, "ortho: left clipping plane same as right one.");

	if ((top - bottom) == 0.0)
		verror(VERR_BADPLANE, "ortho: bottom clipping plane same as top one.");

	if ((yon - hither) == 0.0)
		verror(VERR_BADPLANE, "ortho: near clipping plane same as far one.");
...
ortho2(float left, float right, float bottom, float top)
{
	...
	if (!vdevice.initialised) 
		verror(VERR_UNINIT, "ortho2: vogle not initialised");

	identmatrix(mat);

	if ((right - left) == 0.0)
		verror(VERR_BADPLANE, "ortho2: left clipping plane same as right one.");

	if ((top - bottom) == 0.0)
		verror(VERR_BADPLANE, "ortho2: bottom clipping plane same as top one.");
