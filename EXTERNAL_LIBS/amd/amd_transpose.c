...
/* Sorts, transposes, and removes duplicate entries from the nonzero pattern of
 * a column-form matrix A, to obtain the matrix R.
 * See amd.h for a complete description of AMD_transpose.
 */

#include "amd_internal.h"

GLOBAL Int AMD_transpose    /* returns AMD_OK if input is OK, AMD_INVALID
...
)
{
    ...
    if (!AMD_transpose_valid (n, Ap, Ai) || !Ri || !Rp)
    {
	return (AMD_INVALID) ;
    }
    ...
    AMD_wtranspose (n, Ap, Ai, Rp, Ri, W, Flag) ;
...
/* The AMD_wtranspose routine is not user-callable.  It does not check its
 * input for errors or allocate workspace (that is done by the user-callable
 * AMD_transpose routine).  It does handle the n=0 case. */

GLOBAL void AMD_wtranspose
(
    Int n,		/* input matrix: A is n-by-n */
    const Int Ap [ ],	/* size n+1 */
    const Int Ai [ ],	/* size nz = Ap [n] */
 ...
 * to AMD_wtranspose, FALSE otherwise. */

GLOBAL Int AMD_transpose_valid
(
    Int n,
    const Int Ap [ ],
    const Int Ai [ ]
