 ...
 * following type.
 */
#define IMAGE_FN(fn) float (fn)(float x, float t)
/*
 ...
 */
  XtAddEventHandler(im->w_data, PointerMotionMask, False, report_cursor,  /*(XtPointer)im->w_coord*/(XtPointer)im);
/*
 ...
 */
  ...
  do
  {
     ...
     {
        im->data[i] = im->fn( im->coords[i] , im->iter / 50.0 );
     };
    ...
    */
     ...
     report_cursor( im->w_data, im, xxx  , False);
  }
  while ( im->stop == 0 );
...
static IMAGE_FN(gaus_fn)
{
  return exp(-sqrt(x*x)/2.0f)*cos(t);
}
...
static IMAGE_FN(star_fn)
{
  return ( 1.0 + sin(x)) * exp(-(x*x)/10.0)*cos(t) ;
}
 ...
 */
    if(xmp_pixel_to_world(w, event->xmotion.x, event->xmotion.y, &wx, &wy) == 0)
    {
      ...
      XtVaSetValues(/*w*/im->w_coord,
		    XtVaTypedArg, XmNlabelString,
		    XmRString, text, strlen(text)+1,
		    NULL);
    };

    if ( ((10.0<wx)&&(wx<11.0)) && ((4.0<wy)&&(wy<5.0)) )
   {
       static int count = 0;
       printf("inside <STOP>! (%d)\n", count++);
