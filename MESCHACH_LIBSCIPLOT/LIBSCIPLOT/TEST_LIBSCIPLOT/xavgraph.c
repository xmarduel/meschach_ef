...
static void xav_dump_points(Point *p, int no_of_points);


static char ** xav_extract_argc_and_argv(const char* command, int *argc)
{
   /* the returned value */
   char **xav_argv = malloc( 250*sizeof(char*) );
   ...
   *argc = 0;
   ...
   while ( !finished )
   {
      ...
      strncpy(tmp, command+i1,i2-i1+1);
      tmp[i2-i1] = '\0';

      /*printf("word %d -> %s\n",*argc,tmp);*/

      xav_argv[*argc] = strdup(tmp);
...
int main (int argc, char **argv)
{
  ...
  char command1[] = "graph -T gif -C -L \"title-1\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat";

  char command2[] = "graph -T X -C \
--reposition .0 .0 .5 -w 1.65 -h 0.65 -L \"title-1\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat \
--reposition .0 .5 .5 -w 1.65 -h 0.65 -L \"title-2\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat";

   char command4[] = "graph -T X -C \
--reposition .0 .0 .5 -w 0.65 -h 0.65 -L \"title-1\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat \
--reposition .0 .5 .5 -w 0.65 -h 0.65 -L \"title-2\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat \
--reposition .5 .0 .5 -w 0.65 -h 0.65 -L \"title-3\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat \
--reposition .5 .5 .5 -w 0.65 -h 0.65 -L \"title-4\" -X \"X-axis\" -Y \"Y-axis\" -x 0 7 -y 0 30 graph_driver_file.dat";


  argv = xav_extract_argc_and_argv(command2, &argc);
	  ...
	  if (filter)
	      ...
	      else	/* not first file of plot, but do some things anyway */
		{
	      ...
	      read_file (reader, &p, &points_length, &no_of_points);
              ...
              xav_dump_points(p, no_of_points);
...
void xav_dump_points(Point *p, int no_of_points)
{
   int i;
   for (i=0; i<no_of_points; i++)
   {
      ...
      printf("p[%d].have_x_errorbar = %d\n",i,p[i].have_x_errorbar);
      printf("p[%d].have_y_errorbar = %d\n",i,p[i].have_y_errorbar);
      ...
      printf("p[%d].pendown = %d\n",i,p[i].pendown);

      printf("p[%d].symbol = %d\n",i,p[i].symbol);
      printf("p[%d].symbol_size = %lf\n",i,p[i].symbol_size);
      printf("p[%d].symbol_font_name = %s\n",i,p[i].symbol_font_name);

      printf("p[%d].linemode = %d\n",i,p[i].linemode);
      printf("p[%d].line_width = %lf\n",i,p[i].line_width);

      printf("p[%d].fill_fraction = %lf\n",i,p[i].fill_fraction);
      printf("p[%d].use_color = %d\n",i,p[i].use_color);
   }
}
