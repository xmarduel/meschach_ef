...
class Parser(object):
    ...
    def format(self):
        ...
        try:
            for xtoken in tokenize.generate_tokens(text.readline):
                # unpack the components of each token
                toktype, toktext, (srow, scol), (erow, ecol), line = xtoken
                if False:  # You may enable this for debugging purposes only
                    print "type", toktype, xtoken.tok_name[toktype],
                    print "text", toktext,
...
if __name__ == "__main__":
    ...
    Parser(source, open('utils.html', 'wt')).format()
    # Load HTML page into browser
    import webbrowser
    webbrowser.open("utils.html")
