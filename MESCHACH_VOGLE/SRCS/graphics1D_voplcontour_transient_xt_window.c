...
/** \file graphics1D_voplcontour_transient_xt_window.c
 *
...
WIDGET_USERDATA_STRUCT * widgetUserData; /* static data */
...
{
   VOPL_GRAPH_DATA* pdata = (VOPL_GRAPH_DATA *)data;
...
static void drawscene (const VOPL_GRAPH_DATA *pdata)
{
   ...
   for (k=0; k<nx*ny; k++)
   {
		((VOPL_CONTOUR_DATA *)pdata->dataplot[k])->delete_objects(pdata->dataplot[k]);
		((VOPL_CONTOUR_DATA *)pdata->dataplot[k])->setup_objects(pdata->dataplot[k]);
...
static void generic_output(WIDGET_USERDATA_STRUCT *xawUserData, VOGLE_OUTPUT_FORMAT output_format)
{
   ...
   vopl_winset(xawUserData->xaw_win->vogle_win);
}
...
static void resize(Widget w, XEvent *ev, String *params, Cardinal *n_params)
{
   ...
   vo_xt_win_size(widgetUserData->xaw_win->vogle_win, (int)l, (int)h);
   calcviewport();
...
static void do_ppm_output(Widget w, XtPointer data, XtPointer call_data) /* Xaw callback */
{
   ...
   if ( widgetdata->xaw_win->stop == 1 )
   {
	...
	widgetdata->xaw_win->film_output_type = VOGLEe_PPM;
}
...
static void do_gif_output(Widget w, XtPointer data, XtPointer call_data) /* Xaw callback */
{
   ...
   if ( widgetdata->xaw_win->stop == 1 )
   {
	...
	widgetdata->xaw_win->film_output_type = VOGLEe_GIF;
}
...
static void do_tiff_output(Widget w, XtPointer data, XtPointer call_data) /* Xaw callback */
{
   ...
   if ( widgetdata->xaw_win->stop == 1 )
   {
	...
	widgetdata->xaw_win->film_output_type = VOGLEe_TIFF; /* if "film" set "on", the program will be quite slow... */
}
...
static void do_xbm_output(Widget w, XtPointer data, XtPointer call_data) /* Xaw callback */
{
   ...
   if ( widgetdata->xaw_win->stop == 1 )
   {
	...
	widgetdata->xaw_win->film_output_type = VOGLEe_XBM; /* if "film" set "on", the program will be quite slow... */
}
...
static void do_png_output(Widget w, XtPointer data, XtPointer call_data) /* Xaw callback */
{
   ...
   if ( widgetdata->xaw_win->stop == 1 )
   {
	...
	widgetdata->xaw_win->film_output_type = VOGLEe_PNG; /* if "film" set "on", the program will be quite slow... */
}
 ...
 * 	Init a xt-window 
 */
void * vopl_contours_with_xt_toolkit_transient(void* data)
{
