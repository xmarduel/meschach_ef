...
/* RCS id: $Id: sparse.h,v 1.4 2004/05/31 15:20:01 xavier Exp $ */
...
/* from 'spbkp.c' */
extern  int	unord_get_idx(SPROW	*r, int j);
extern  double	unord_get_val(SPMAT	*A, int i, int j);
extern  row_elt	*bkp_bump_col(SPMAT	*A, int j, int *row, int *idx);
extern  SPMAT	*bkp_interchange(SPMAT	*A, int i1, int i2);
extern  int	iv_min(IVEC	*iv, int *index);

/* from 'spswap.c' */
extern  void patch_col(SPMAT *A, int col, int old_row, int old_idx, int row_num, int idx);

/* from 'spchfctr' */
extern  double	sprow_ip(SPROW *row1, SPROW *row2, int lim);
