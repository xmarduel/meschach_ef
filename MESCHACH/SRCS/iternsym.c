...
#ifdef ANSI_C
extern  VEC	*spCHsolve(SPMAT *sp, VEC *x, VEC *z);
#else
VEC	*spCHsolve();
#endif
...
extern  VEC	*iter_cgs(ITER *ip, VEC *r0)
{
...
*/
extern  VEC *iter_spcgs(SPMAT *A,SPMAT *B, VEC *b, VEC *r0, double tol, VEC *x, int limit, int *steps)
{	
   ITER *ip;
...
extern  VEC	*iter_splsqr(SPMAT *A, VEC *b, double tol, VEC *x, int limit, int *steps)
{
   ITER *ip;
...
*/
extern  MAT	*iter_arnoldi_iref(ITER *ip, Real *h_rem, MAT *Q, MAT *H)
{
...
*/
extern  MAT	*iter_arnoldi(ITER *ip, Real *h_rem, MAT *Q, MAT *H)
{
...
extern  MAT	*iter_sparnoldi(SPMAT *A,VEC *x0, int m, Real *h_rem, MAT *Q, MAT *H)
{
   ITER *ip;
...
extern  VEC *iter_mgcr(ITER *ip)
{
   ...
   for (ip->steps = 0; ip->steps < ip->limit; ) {
	...
	break;  /* residual is zero -> finish the process */ 
	 ...
	 if (fabs((double)alpha->ve[i]) > dd)  {     /* do restart */
	 ...
	 /* nres = dd*dd - alpha->ve[i]*alpha->ve[i];
	 dd = nres = sqrt((double) nres);  */

	 /* to avoid overflow in computing dd */
...
/* no preconditioner */
extern  VEC	*iter_spmgcr(SPMAT *A, SPMAT *B, VEC *b, double tol, VEC *x, int k, int limit, int *steps)
{
   ITER *ip;
...
/* iter_spcgne -- a simple interface to iter_cgne() which 
   ...
   In the second case the solution vector is created.
*/
extern  VEC  *iter_spcgne(SPMAT *A, SPMAT *B, VEC *b, double eps, VEC *x, int limit, int *steps)
{	
   ITER *ip;
