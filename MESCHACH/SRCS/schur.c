...
static char rcsid[] = "$Id: schur.c,v 1.1 2003/08/09 16:25:32 xavier Exp $";
...
MAT	*schur_vecs(MAT	*T,MAT *Q,MAT *X_re,MAT *X_im)
{
	...
	if ( ! T || ! X_re ) error(E_NULL,"schur_vecs");
	if ( T->m != T->n || X_re->m != X_re->n ||
		( Q != MNULL && Q->m != Q->n ) ||
		( X_im != MNULL && X_im->m != X_im->n ) ) error(E_SQUARE,"schur_vecs");
	if ( T->m != X_re->m ||
		( Q != MNULL && T->m != Q->m ) ||
		( X_im != MNULL && T->m != X_im->m ) ) error(E_SIZES,"schur_vecs");
	...
	while ( i < T->m )
	{
	  ...
	  /* printf("limit = %d\n",limit); */
	  for ( j = limit+1; j < T->m; j++ ) tmp1_re->ve[j] = 0.0;
	  j = limit;
	  while ( j >= 0 )
	    {
	    ...
	    if ( l_im != 0.0 ) sv_mlt(1/norm,tmp1_im,tmp1_im);
	    mv_mlt(Q,tmp1_re,tmp2_re);
	    if ( l_im != 0.0 ) mv_mlt(Q,tmp1_im,tmp2_im);
	    if ( l_im != 0.0 ) norm = sqrt(in_prod(tmp2_re,tmp2_re)+in_prod(tmp2_im,tmp2_im));
	    else norm = v_norm2(tmp2_re);
	    
	    sv_mlt(1/norm,tmp2_re,tmp2_re);
	    if ( l_im != 0.0 ) sv_mlt(1/norm,tmp2_im,tmp2_im);

	    if ( l_im != 0.0 )
	    {
		   if ( ! X_im ) error(E_NULL,"schur_vecs");
	    ...
	    {
		   set_col(X_re,i,tmp2_re);
		   if ( X_im != MNULL ) set_col(X_im,i,tmp1_im);	/* zero vector */
		   i += 1;
	    }
	} /* end while */
