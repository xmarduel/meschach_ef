...
extern  int	fy_or_n(FILE *fp, char *s)
{
	char	*cp;
...
extern  int	fin_int(FILE *fp, char *s, int low, int high)
{
	int	retcode, x;
...
/* fin_double -- return double read from file/stream fp
	...
	-- check that x lies between low and high: re-prompt if
		fp is a tty, error exit otherwise
	-- ignore check if low > high		*/
extern  double	fin_double(FILE *fp, char *s, int low, int high)
{
	Real	retcode, x;
