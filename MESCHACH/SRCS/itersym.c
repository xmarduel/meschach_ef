...
#ifdef ANSI_C
extern  VEC	*spCHsolve(SPMAT *,VEC *,VEC *);
...
/* iter_spcg -- a simple interface to iter_cg() which uses sparse matrix
   ...
   */
extern  VEC  *iter_spcg(SPMAT *A, SPMAT *LLT, VEC *b, double eps, VEC *x, int limit, int *steps)
{	
   ITER *ip;
...
extern  void	iter_lanczos(ITER *ip, VEC *a, VEC *b, Real *beta2, MAT *Q)
{
   int	j;
...
extern  void iter_splanczos(SPMAT *A, int m, VEC *x0, VEC *a, VEC *b, Real *beta2, MAT *Q)
{	
   ITER *ip;
...
extern  VEC	*iter_lanczos2(ITER *ip, VEC *evals, VEC *err_est)
/* ITER structure */
...
#ifdef CODE_WARRIOR   
   qsort((void *)(a->ve),(size_t)(a->dim), (size_t)sizeof(Real),(_Cmpfun*)dbl_cmp);
#else
   qsort((void *)(a->ve),(int)(a->dim),sizeof(Real),
	 (int (*)(const void *, const void *))dbl_cmp);
#endif
...
extern  VEC    *iter_splanczos2(SPMAT *A, int m, VEC *x0, VEC *evals, VEC *err_est)
/* initial vector */
/* eigenvalue vector */
