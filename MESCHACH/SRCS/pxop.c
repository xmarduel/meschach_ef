...
static	char	rcsid[] = "$Id: pxop.c,v 1.2 2003/12/08 20:05:20 xavier Exp $";
...
extern  PERM	*px_inv(PERM *px, PERM *out)
{
    int	i, j, k, n, *p;
...
extern  VEC	*px_vec(PERM *px, VEC *vector, VEC *out)
{
...
extern  VEC	*pxinv_vec(PERM *px, VEC *x, VEC *out)
{
    u_int	i, size;
...
extern  PERM	*px_transp(PERM *px, u_int i1, u_int i2)
/* permutation to transpose */
...
extern  int	px_sign(PERM *px)
{
	...
	numtransp = myqsort((int*)px2->pe, (int)px2->size);
	px_free(px2);
...
/* px_cols -- permute columns of matrix A; out = A.px'
	-- May NOT be in situ */
extern  MAT	*px_cols(PERM *px, MAT *A, MAT *out)
{
	int	i, j, m, n, px_j;
	Real	**A_me, **out_me;
#ifdef ANSI_C
