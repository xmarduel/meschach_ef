...
static char rcsid[] = "$Id: matlab.c,v 1.2 2003/12/08 20:05:20 xavier Exp $";

/* m_save -- save matrix in ".mat" file for MATLAB
	-- returns matrix to be saved */
extern  MAT     *m_save(FILE *fp, MAT *A, char *name)
{
	int     i;
	matlab  mat;
...
extern  VEC     *v_save(FILE *fp, VEC *x, char *name)
{
	matlab  mat;
...
/* d_save -- save double in ".mat" file for MATLAB
	-- saves it as a row vector
	-- returns vector to be saved */
extern  double	d_save(FILE *fp, double x, char *name)
{
	matlab  mat;
	Real x1 = x;
...
extern  MAT     *m_load(FILE *fp, char **name)
{
	MAT     *A;
	int     i;
