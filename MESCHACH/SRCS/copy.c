...
static	char	rcsid[] = "$Id: copy.c,v 1.2 2003/12/08 20:05:20 xavier Exp $";
...
extern  MAT	*m_move(MAT	*in, int i0, int j0, int m0, int n0, MAT *out, int i1, int j1)
{
    int		i;
...
extern  VEC	*v_move(VEC	*in, int i0, int  dim0, VEC	*out, int i1)
{
    if ( ! in )
	error(E_NULL,"v_move");
    if ( i0 < 0 || dim0 < 0 || i1 < 0 ||
...
/* mv_move -- copies selected piece of matrix to a vector
	...
	-- out is resized if necessary */
extern  VEC	*mv_move(MAT *in, int i0, int j0, int m0, int n0, VEC *out, int i1)
{
    int		dim1, i;
...
/* vm_move -- copies selected piece of vector to a matrix
	...
	-- out is resized if necessary */
extern  MAT	*vm_move(VEC *in, int i0, MAT	*out, int i1, int j1, int m1, int n1)
{
    int		dim0, i;
