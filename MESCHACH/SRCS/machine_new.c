...
{
#ifdef MACOSX_CBLAS
   return cblas_ddot(len, dp1, 1, dp2, 1);
#else
   register int	len4;
   register int	i;
...
extern  void	__smlt__(register double *dp,register double s, register double	*out, register int len)
{
   ...
   if ( dp == out )
   {
      cblas_dscal(len,     s, out, 1);
   }
   else
   {
      cblas_dcopy(len, dp, 1, out, 1);
      cblas_dscal(len,     s, out, 1);
   }
#else
   register int	i;
...
extern  void	__add__(register double	*dp1, register double *dp2,register double *out,register int len)
{
   ...
   if ( dp1 == out )
   {
      cblas_daxpy(len, 1.0, dp2, 1, out, 1);
   }
   else
   if ( dp2 == out )
   {
      cblas_daxpy(len, 1.0, dp1, 1, out, 1);
   }
   else
   {
      cblas_dcopy(len, dp1, 1, out, 1);
      cblas_daxpy(len, 1.0, dp2, 1, out, 1);
   }
#else
   register int	i;
