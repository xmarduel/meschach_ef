...
static char	rcsid[] = "$Id: zzz_sprow.old.c,v 1.1 2003/08/09 16:25:32 xavier Exp $";
...
extern  void sprow_dump(FILE *fp, SPROW *r)
{
   int  j_idx;
   row_elt *elts;
...
extern  SPROW	*sprow_get(int maxlen)
{
   SPROW	*r;
...
/* sprow_xpd -- expand row by means of realloc()
   ...
   -- returns r */
extern  SPROW	*sprow_xpd(SPROW *r, int n, int type)
{
   int	newlen;
...
/* sprow_resize -- resize a SPROW variable by means of realloc()
   -- n is a new size
   -- returns r */
extern  SPROW	*sprow_resize(SPROW	*r, int n, int type)
{
   if (n < 0)
...
/* sprow_merge -- merges r1 and r2 into r_out
   ...
   -- type must be SPMAT or SPROW depending on
      ...
      or a SPROW variable
   -- returns r_out */
extern  SPROW	*sprow_merge(SPROW	*r1,SPROW	*r2,SPROW	*r_out,int type)
{
...
/* sprow_copy -- copies r1 and r2 into r_out
   ...
   -- type must be SPMAT or SPROW depending on
      ...
      or a SPROW variable
   -- returns r_out */
extern  SPROW	*sprow_copy(SPROW *r1, SPROW *r2, SPROW *r_out, int type)
{
   int	idx1, idx2, idx_out, len1, len2, len_out;
   row_elt	*elt1, *elt2, *elt_out;
...
/* sprow_mltadd -- sets r_out <- r1 + alpha.r2
   ...
   -- type must be SPMAT or SPROW depending on
      ...
      or a SPROW variable
   -- returns r_out */
extern  SPROW	*sprow_mltadd(SPROW	*r1, SPROW	*r2, double alpha, int j0, SPROW	*r_out, int type)
{
   int	idx1, idx2, idx_out, len1, len2, len_out;
   row_elt	*elt1, *elt2, *elt_out;
...
/* sprow_add -- sets r_out <- r1 + r2
   ...
   -- type must be SPMAT or SPROW depending on
      ...
      or a SPROW variable
   -- returns r_out */
extern  SPROW	*sprow_add(SPROW *r1, SPROW	*r2, int j0, SPROW	*r_out, int type)
{
   int	idx1, idx2, idx_out, len1, len2, len_out;
   row_elt	*elt1, *elt2, *elt_out;
...
/* sprow_sub -- sets r_out <- r1 - r2
   ...
   -- type must be SPMAT or SPROW depending on
      ...
      or a SPROW variable
   -- returns r_out */
extern  SPROW	*sprow_sub(SPROW *r1, SPROW *r2, int j0, SPROW *r_out, int type)
{
   int	idx1, idx2, idx_out, len1, len2, len_out;
   row_elt	*elt1, *elt2, *elt_out;
...
/* sprow_smlt -- sets r_out <- alpha*r1 
   ...
   -- only for columns j0, j0+1, ...
   -- returns r_out */
extern  SPROW	*sprow_smlt(SPROW *r1, double alpha, int j0, SPROW *r_out, int type)
{
...
extern  void	sprow_foutput(FILE *fp, SPROW *r)
{
   int	i, len;
   row_elt	*e;
...
extern  double  sprow_set_val(SPROW *r, int j, double val)
{
