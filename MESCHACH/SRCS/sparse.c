...
extern  double	sp_get_val(const SPMAT *A, int i, int j)
{
   SPROW	*r;
   int	idx;
...
extern  double	sp_set_val(SPMAT *A, int i, int j, double val)
{
   SPROW	*r;
...
extern  VEC	*sp_mv_mlt(SPMAT	*A, VEC *x, VEC *out)
{
...
extern  VEC	*sp_vm_mlt(SPMAT *A, VEC *x, VEC *out)
{
   int	i, j_idx, m, n, max_idx;
...
extern  SPMAT	*sp_get(int m, int n, int maxlen)
{
   SPMAT	*A;
   SPROW	*rows;
   int	i;
...
/* sp_col_access -- set column access path; i.e. nxt_row, nxt_idx fields
   -- returns A */
extern  SPMAT *sp_col_access(SPMAT *A)
{
...
extern  SPMAT *sp_diag_access(SPMAT *A)
{
   int	i, m;
   SPROW	*row;
...
extern  MAT	*sp_m2dense(SPMAT *A, MAT *out)
{
   int	i, j_idx;
   SPROW	*row;
   row_elt	*elt;
...
extern  SPMAT *sp_add(SPMAT *A, SPMAT *B, SPMAT *C)
{
   int i, in_situ;
   SPROW *rc;
   static SPROW *tmp;
...
extern  SPMAT *sp_sub(SPMAT *A,SPMAT *B,SPMAT *C)
{
   int i, in_situ;
   SPROW *rc;
   static SPROW *tmp;
...
extern  SPMAT *sp_mltadd(SPMAT *A, SPMAT *B, double alpha, SPMAT *C)
{
   int i, in_situ;
   SPROW *rc;
   static SPROW *tmp;
...
extern  SPMAT *sp_smlt(SPMAT *A, double alpha, SPMAT *B)
{
   int i;
...
extern  SPMAT	*sp_copy2(SPMAT *A, SPMAT *OUT)
{
   int	i /* , idx, len1, len2 */;
   SPROW	*r1, *r2;
...
extern  SPMAT	*sp_resize(SPMAT *A, int m, int n)
{
   int	i, len;
   SPROW	*r;
...
extern  SPMAT	*sp_compact(SPMAT *A, double tol)
{
   int	i, idx1, idx2;
   SPROW	*r;
   row_elt	*elt1, *elt2;
...
extern  int sp_get_vars(int m,int n,int deg,...) 
{
   va_list ap;
   int i=0;
   SPMAT **par;
...
extern  int sp_resize_vars(int m,int n,...) 
{
   va_list ap;
   int i=0;
   SPMAT **par;
...
extern  int sp_free_vars(SPMAT **va,...)
{
   va_list ap;
   int i=1;
   SPMAT **par;
...
extern  int sp_get_vars(va_alist) va_dcl
{
   va_list ap;
   int i=0, m, n, deg;
   SPMAT **par;
...
extern  int sp_resize_vars(va_alist) va_dcl
{
   va_list ap;
   int i=0, m, n;
   SPMAT **par;
...
extern  int sp_free_vars(va_alist) va_dcl
{
   va_list ap;
   int i=0;
   SPMAT **par;
