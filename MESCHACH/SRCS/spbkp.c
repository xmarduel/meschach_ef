...
static	char	rcsid[] = "$Id: spbkp.c,v 1.2 2003/10/12 15:42:37 xavier Exp $";
...
extern  int	chk_col_access(SPMAT	*A)
{
...
extern  SPMAT	*spBKPfactor(SPMAT *A, PERM *pivot, PERM *blocks, double tol)
{
    ...
    for ( i = 0; i < A->m; i++ )
      qsort(A->row[i].elt,A->row[i].len,sizeof(row_elt),
	    (int(*)(const void*,const void*))col_cmp);
...
extern  VEC	*spBKPsolve(SPMAT *A, PERM *pivot, PERM *block, VEC *b, VEC *x)
{
