...
extern  BAND *bd_get(int lb,int  ub,int  n)
{
   BAND *A;
...
extern  BAND *bd_resize(BAND *A,int new_lb, int new_ub, int new_n)
{
...
extern  BAND *bd_copy(BAND *A, BAND *B)
{
   int lb,ub,i,j,n;
...
extern  MAT *band2mat(BAND *bA, MAT *A)
{
   int i,j,l,n,n1;
   int lb, ub;
   Real **bmat;
...
/* copy a square matrix to a band matrix with 
   lb subdiagonals and ub superdiagonals */
extern  BAND *mat2band(MAT *A,int lb, int ub, BAND *bA)
{
   int i, j, l, n1;
   Real **bmat;
...
/* bdLUfactor -- gaussian elimination with partial pivoting
   ...
   */ 
extern  BAND	*bdLUfactor(BAND *bA, PERM *pivot)
{
...
extern  VEC	*bdLUsolve(BAND	*bA, PERM *pivot, VEC *b,VEC *x)
{
...
extern  BAND *bdLDLfactor(BAND *A)
{
...
extern  VEC *bdLDLsolve(BAND *A, VEC *b, VEC *x)
{
...
extern  VEC *bd_mv_mlt(BAND *A, VEC *x, VEC *out)
{
  int i, j, j_end, k;
