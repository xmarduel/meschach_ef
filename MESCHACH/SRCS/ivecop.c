...
extern  IVEC	*iv_resize(IVEC	*iv, int new_dim)
{
   int	i;
...
extern  IVEC *iv_move(IVEC *in, int i0, int dim0, IVEC *out, int i1)
{
    if ( ! in )
...
extern  IVEC *iv_add(IVEC *iv1, IVEC *iv2, IVEC *out)
{
   u_int	i;
...
extern  IVEC	*iv_sub(IVEC *iv1, IVEC *iv2, IVEC *out)
{
   u_int	i;
   int	*out_ive, *iv1_ive, *iv2_ive;
...
extern  void	iv_foutput(FILE	*fp, IVEC *iv)
{
   int	i;
...
extern  IVEC	*iv_finput(FILE	*fp, IVEC *x)
{
   if ( isatty(fileno(fp)) )
...
extern  void	iv_dump(FILE *fp, IVEC *iv)
{
   int		i;
...
extern  IVEC	*iv_sort(IVEC *x, PERM *order)
{
   int		*x_ive, tmp, v;
   /* int		*order_pe; */
