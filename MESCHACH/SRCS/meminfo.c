...
void mem_dump_list(FILE *fp, int list)
{
   ...
   fprintf(fp," %15s[%d] (@ 0x%lx):\n","CONTENTS OF mem_connect",list,mlist);
...
void mem_info_file(FILE *fp, int list)
{
   ...
   if (list == 0)
      ...
      fprintf(fp," MEMORY INFORMATION (list no. %d (@ 0x%lx) ):\n",list,mlist);
...
/* print out memory info to the file fp, only for ONE type */
