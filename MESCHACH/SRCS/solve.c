...
static	char	rcsid[] = "$Id: solve.c,v 1.1 2003/08/09 16:25:32 xavier Exp $";
...
extern  VEC	*Usolve(MAT	*matrix, VEC *b, VEC *out, double diag)
{
	u_int	dim /* , j */;
	int	i, i_lim;
...
extern  VEC	*Lsolve(MAT	*matrix, VEC *b, VEC *out, double diag)
{
	u_int	dim, i, i_lim /* , j */;
	Real	**mat_ent, *mat_row, *b_ent, *out_ent, *out_col, sum;
...
extern  VEC	*UTsolve(MAT *U, VEC *b, VEC *out, double diag)
{
    u_int	dim, i, i_lim;
...
extern  VEC	*Dsolve(MAT	*A, VEC *b, VEC *x)
{
    u_int	dim, i;
...
/* LTsolve -- back substitution with optional over-riding diagonal
		...
		-- can be in-situ but doesn't need to be */
extern  VEC	*LTsolve(MAT *L, VEC *b, VEC *out, double diag)
{
    u_int	dim;
    int		i, i_lim;
