...
extern  MAT *m_finput(FILE *fp, MAT *a)
{
     if ( isatty(fileno(fp)) )
...
extern  PERM    *px_finput(FILE *fp, PERM *px)
{
     if ( isatty(fileno(fp)) )
...
extern  VEC     *v_finput(FILE *fp, VEC *x)
{
     if ( isatty(fileno(fp)) )
...
extern  char	*setformat(char    *f_string)
{
    char	*old_f_string;
    old_f_string = format;
...
extern  void    m_foutput(FILE *fp, MAT *a)
{
     u_int      i, j, tmp;
...
extern  void    px_foutput(FILE *fp, PERM *px)
{
     u_int      i;
...
extern  void    v_foutput(FILE *fp, VEC *x)
{
     u_int      i, tmp;
...
extern  void  m_dump(FILE *fp, MAT *a)
{
	u_int   i, j, tmp;
...
extern  void  px_dump(FILE *fp, PERM *px)
{
     u_int      i;
...
extern  void  v_dump(FILE *fp, VEC *x)
{
     u_int      i, tmp;
