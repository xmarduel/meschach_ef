...
extern  void iter_std_info(ITER *ip, double nres, VEC *res, VEC *Bres)
{
   if (nres >= 0.0)
     printf(" %d. residual = %g\n",ip->steps,nres);
   else 
...
extern  int iter_std_stop_crit(ITER *ip, double nres, VEC *res, VEC *Bres)
{
...
extern  ITER *iter_get(int lenb, int lenx)
{
   ITER *ip;
...
extern  ITER *iter_resize(ITER *ip, int new_lenb, int new_lenx)
{
   int old;
...
extern  void iter_dump(ITER *ip, FILE* fp)
{
   if (ip == NULL) {
...
*/
extern  ITER *iter_copy2(ITER *ip1, ITER *ip2)
{
   VEC *x, *b;
   int shx, shb;
...
extern  ITER *iter_copy(ITER *ip1, ITER *ip2)
{
   VEC *x, *b;
...
/* iter_gen_sym -- generate symmetric positive definite
   ...
   */
extern  SPMAT	*iter_gen_sym(int n, int nrow)
{
   SPMAT	*A;
   VEC	        *u;
   Real       s1;
...
*/
extern  SPMAT	*iter_gen_nonsym(int m, int n, int nrow, double diag)
{
   SPMAT	*A;
   PERM		*px;
   int		i, j, k, k_max;
...
/* iter_gen_nonsym -- generate non-symmetric positive definite 
   ...
   nrow - number of entries in a row
*/
extern  SPMAT	*iter_gen_nonsym_posdef(int n, int nrow)
{
   SPMAT	*A;
   PERM		*px;
   VEC          *u;
