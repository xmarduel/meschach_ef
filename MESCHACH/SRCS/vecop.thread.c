...
#ifdef XAV_WITH_PTHREADS
#include <pthread.h>
#endif

static	char	rcsid[] = "$Id: vecop.thread.c,v 1.1 2006/10/07 14:56:46 xavier Exp $";
...
typedef struct v_add_threads_package_t_
{
   int id;
...
} v_add_threads_package_t;

void  calculate_v_add_threads_package_datas(const int total_len, const int nb_threads, int vec_add_start[], int vec_add_lengh[] )
{
   int i;

   int lenght_1  = floor( total_len / nb_threads );
   int lenght_2  = lenght_1 + 1;

   int nb_threads_with_len_2 = total_len  % nb_threads;              /* there will be nb_threads_with_len_2 with lenght_2 */
   int nb_threads_with_len_1 = nb_threads - nb_threads_with_len_2 ;  /* there will be nb_threads_with_len_1 with lenght_1 */

   int total_len_with_lenght_1 = nb_threads_with_len_1*lenght_1;
   
   for (i=0; i<nb_threads_with_len_1; i++)
   {
      vec_add_start[i] = i*lenght_1;
      vec_add_lengh[i] =   lenght_1;
   }

   for (i=0; i<nb_threads_with_len_2 ; i++)
   {
      vec_add_start[i+nb_threads_with_len_1] = total_len_with_lenght_1 + (i-nb_threads_with_len_1)*(lenght_2);
      vec_add_lengh[i+nb_threads_with_len_1] = lenght_2;
   }

   
#ifdef XAV_WITH_PTHREADS_DEBUG
   printf( "NB_THREADS = %d \n", nb_threads);
...
void *v_add_partial(void *vec_package)
{
   v_add_threads_package_t* pack = (v_add_threads_package_t*)vec_package;

   double* v1 = pack->vec1->ve + pack->start;
   double* v2 = pack->vec2->ve + pack->start;
   double* v3 = pack->vec3->ve + pack->start;
   
#ifdef XAV_WITH_PTHREADS_DEBUG
   printf("thread No %d ...\n", pack->id);
#endif

    __add__(v1, v2, v3, pack->lengh); 
...
{
   if ( vec1==(VEC *)NULL || vec2==(VEC *)NULL )     error(E_NULL,"v_add");
   if ( vec1->dim != vec2->dim )                     error(E_SIZES,"v_add");
   if ( out==(VEC *)NULL || out->dim != vec1->dim )  out = v_resize(out,vec1->dim);
   
#ifdef XAV_WITH_PTHREADS
   pthread_t                 thread[NB_THREADS];
   v_add_threads_package_t   vec_package[NB_THREADS];

   int          vec_add_start[NB_THREADS];
   int          vec_add_lenght[NB_THREADS];

   int k,k1,k2;

   calculate_v_add_threads_package_datas(vec1->dim, NB_THREADS, vec_add_start, vec_add_lenght);
   
   for (k=0; k<NB_THREADS; k++)
   {
      vec_package[k].id    = k;

      vec_package[k].vec1  = vec1;
      vec_package[k].vec2  = vec2;
      vec_package[k].vec3  = out;
      vec_package[k].start = vec_add_start[k];
      vec_package[k].lengh = vec_add_lenght[k];
   }
   
   for (k1=0; k1<NB_THREADS; k1++)
   {
      if (pthread_create(&thread[k1], NULL, (void *) v_add_partial , (void *) &vec_package[k1]) != 0) error(E_UNKNOWN,"pthread_create");
   }
 
   for (k2=0; k2<NB_THREADS; k2++)
   {   
      if (pthread_join(thread[k2], NULL) != 0)  error(E_UNKNOWN,"pthread_join");
   }

#else 

    __add__(vec1->ve,vec2->ve,out->ve, (int)vec1->dim); 
...
#ifdef PROTOTYPES_IN_STRUCT
VEC	*_v_map(double (*f)(void *,double),void *params,VEC *x,VEC *out)
#else
