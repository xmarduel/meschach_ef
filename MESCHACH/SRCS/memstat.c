...
static char rcsid[] = "$Id: memstat.c,v 1.3 2004/07/22 12:13:38 xavier Exp $";
...
extern  int mem_stat_reg_list(void **var, int type, int list)
{
   int n;
...
extern  int mem_stat_mark(int mark)
{
   if (mark < 0)
   {
...
extern  int mem_stat_free_list(int mark, int list)
{
   u_int i,j;
   int	 (*free_fn)(void *);
...
extern  void mem_stat_dump(FILE *fp, int list)
{
   u_int i,j,k=1;
...
extern  int mem_stat_reg_vars(int list,int type,...)
{
   va_list ap;
   int i=0;
   void **par;
...
extern  int mem_stat_reg_vars(va_alist) va_dcl
{
   va_list ap;
   int type,list,i=0;
   void **par;
